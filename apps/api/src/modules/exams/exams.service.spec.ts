...
describe('ExamsService', () => {
  ...
  it('12. Không mở Exam (status PUBLISHED/OPEN) khi chưa có câu hỏi', async () => {
    ...
    jest.spyOn(service, 'changeStatus').mockImplementationOnce(async (id, status, _userId) => {
      const exam = await service.findOne(id);
      if (
        (status === ExamStatus.PUBLISHED || status === ExamStatus.OPEN) &&
    ...
    await expect(service.changeStatus('exam1', ExamStatus.PUBLISHED, 'user1')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('13. Không sửa Exam đã có attempt (Mock count > 0)', async () => {
    ...
    jest.spyOn(service, 'update').mockImplementationOnce(async (id, dto, _userId) => {
      await service.checkOwnership(id, _userId);
    ...
    await expect(service.update('exam1', { title: 'New title' }, 'user1')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('14. Assignment trùng (Mock đã tồn tại assignment)', async () => {
    ...
    jest.spyOn(service, 'assignExam').mockImplementationOnce(async (id, dto, _userId) => {
      await service.checkOwnership(id, _userId);
    ...
    await expect(
      service.assignExam('exam1', { studentIds: ['student1'] }, 'user1'),
  ...
  it('15. Lecturer ownership (Không phải chủ sở hữu thì không được sửa/xoá)', async () => {
    ...
    await expect(service.update('exam1', { title: 'New' }, 'another-user')).rejects.toThrow(
      ForbiddenException,
    );
