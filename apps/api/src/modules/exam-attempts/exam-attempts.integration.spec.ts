...
import { ExamStatus, QuestionType, ShowResultMode } from '@prisma/client';
...
describe('ExamAttemptsService (Integration)', () => {
  ...
  beforeEach(async () => {
    ...
    const user = await prisma.user.create({
      data: {
        id: 'u-int-1', email: 'test@int.com', passwordHash: 'hash', fullName: 'Int User'
      }
    });
    userId = user.id;
    ...
    const student = await prisma.student.create({
      data: {
        id: 's-int-1', user: { connect: { id: user.id } }, studentCode: 'SINT1', fullName: 'Int Student', email: 'test@int.com', cohortClass: 'SE101', cohort: 'K18', enrollmentDate: new Date(),
        department: { connectOrCreate: { where: { code: 'DINT1' }, create: { code: 'DINT1', name: 'Int Dept' } } }
      }
    });
    studentId = student.id;

    const course = await prisma.course.create({
      data: { id: 'c-int-1', courseCode: 'INT101', name: 'Int Course', credits: 3, tuitionFeePerCredit: 100, department: { connect: { code: 'DINT1' } } }
    });

    const academicYear = await prisma.academicYear.create({
      data: { id: 'ay-int-1', code: 'AYINT', name: 'Int Year', startDate: new Date('2026-01-01'), endDate: new Date('2026-12-31') }
    });

    const semester = await prisma.semester.create({
      data: { id: 'sem-int-1', code: 'SEMINT', term: 'FIRST', startDate: new Date('2026-01-01'), endDate: new Date('2026-06-30'), name: 'Int Sem', academicYear: { connect: { id: academicYear.id } }, maxCredits: 20, status: 'REGISTRATION_OPEN', registrationStartDate: new Date('2025-12-01'), registrationEndDate: new Date('2025-12-31') }
    });

    const lecturer = await prisma.lecturer.create({
      data: { id: 'l-int-1', lecturerCode: 'LINT', fullName: 'Int Lecturer', email: 'lecturer@int.com', department: { connect: { code: 'DINT1' } }, user: { create: { id: 'u-lect-1', email: 'lecturer@int.com', passwordHash: 'hash', fullName: 'Int Lecturer' } } }
    });

    const classSection = await prisma.classSection.create({
      data: { id: 'cs-int-1', sectionCode: 'CSINT', courseId: course.id, semesterId: semester.id, maxCapacity: 50, lecturerId: lecturer.id, status: 'OPEN' }
    });

    await prisma.enrollment.create({
      data: { id: 'enr-int-1', studentId: student.id, classSectionId: classSection.id, status: 'ENROLLED' }
    });

    const exam = await prisma.exam.create({
      data: {
        id: 'e-int-1', examCode: 'E1', name: 'Exam 1', courseId: course.id, classSectionId: classSection.id, createdByUserId: user.id,
        startsAt: new Date(Date.now() - 100000),
        endsAt: new Date(Date.now() + 100000),
        durationMinutes: 60,
        passScore: 5, questionCount: 1, maxAttempts: 1,
        status: 'OPEN', showResultMode: 'IMMEDIATELY', shuffleQuestions: false, shuffleOptions: false
      }
    });
    examId = exam.id;
    ...
    const question = await prisma.question.create({
      data: {
        id: 'q-int-1', questionCode: 'Q1', courseId: course.id, content: 'Test Q', difficulty: 'EASY', type: 'SINGLE_CHOICE', defaultScore: 10, createdByUserId: user.id,
        options: {
          create: [
  ...
  it('Autosave Concurrency: Should not create duplicate answers', async () => {
    ...
    const snapshot = await prisma.examAttemptQuestion.findFirst({ where: { examAttemptId: attempt.id } });
  ...
  it('Submit and Autosave Concurrency: Should not double-grade', async () => {
    ...
    const notifications = await prisma.notification.findMany({ where: { userId: userId, content: { contains: 'đã nộp' } } });
