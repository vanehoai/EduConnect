    ...
    <div className="max-w-2xl mx-auto space-y-6 mt-8">
      <Card>
        ...
        <CardContent className="space-y-6 text-center">
          <div className="py-8">
            <span className="text-6xl font-bold text-blue-600">{result.score || 0}</span>
            <span className="text-xl text-slate-500"> / {result.totalScore || 100}</span>
          </div>
          
          <div className="grid grid-cols-2 gap-4 text-left border-t pt-6">
            <div>
              <p className="text-sm text-slate-500 mb-1">Thời gian làm bài</p>
              <p className="font-semibold">{result.timeSpent || 0} phút</p>
            </div>
            <div>
              <p className="text-sm text-slate-500 mb-1">Trạng thái</p>
              <p className="font-semibold">{result.isPassed ? <span className="text-green-600 flex items-center gap-1"><CheckCircle className="h-4 w-4"/> Đạt</span> : <span className="text-red-600 flex items-center gap-1"><XCircle className="h-4 w-4"/> Chưa đạt</span>}</p>
            </div>
            <div>
              <p className="text-sm text-slate-500 mb-1">Số câu đúng</p>
              <p className="font-semibold text-green-600">{result.correctCount || 0}</p>
            </div>
            <div>
              <p className="text-sm text-slate-500 mb-1">Số câu sai/bỏ qua</p>
              <p className="font-semibold text-red-600">{result.incorrectCount || 0}</p>
            </div>
          </div>
        </CardContent>
